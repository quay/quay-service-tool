...
def _config(spam_config):
    return dict(spam_config)


def _stored_bytes(config, uri):
    return artifact_storage.get_artifact_storage(config).read(uri)
...
def test_import_preserves_artifact_and_activates_classifier(tmp_path, spam_config):
    ...
    assert _stored_bytes(config, imported["artifact_path"]) == artifact_bytes
    assert imported["artifact_sha256"] == hashlib.sha256(artifact_bytes).hexdigest()


def test_export_preserves_semantically_identical_imported_artifact_bytes(tmp_path, spam_config):
    config = _config(spam_config)
    ...
    assert _stored_bytes(config, exported["artifact_path"]) == artifact_bytes
    assert exported["artifact_sha256"] == hashlib.sha256(artifact_bytes).hexdigest()


def test_import_is_idempotent_for_same_version_and_content(tmp_path, spam_config):
    config = _config(spam_config)
...
def test_import_is_idempotent_after_classifier_is_retrained(tmp_path, spam_config):
    config = _config(spam_config)
    artifact_bytes = json.dumps(_artifact()).encode("utf-8")
...
def test_import_rejects_same_version_with_different_content(tmp_path, spam_config):
    config = _config(spam_config)
...
def test_retraining_imported_classifier_combines_base_with_feedback_once(tmp_path, spam_config):
    config = _config(spam_config)
    imported, _ = classifier.import_classifier_artifact(
        config,
        "Imported classifier",
...
def test_training_writes_quay_compatible_artifact_and_sha(tmp_path, spam_config):
    config = _config(spam_config)
    ...
    artifact_bytes = _stored_bytes(config, trained["artifact_path"])
...
def test_training_rejects_corpus_below_minimum_label_counts(tmp_path, spam_config):
    config = _config(spam_config)
...
def test_training_uses_active_policy_ingress_threshold(tmp_path, spam_config):
    config = _config(spam_config)
    created = store.create_classifier(config, {"name": "test", "enabled": True})
    ...
    artifact = json.loads(_stored_bytes(config, trained["artifact_path"]))
...
def test_export_refreshes_active_policy_ingress_threshold(tmp_path, spam_config):
    config = _config(spam_config)
    created = store.create_classifier(config, {"name": "test", "enabled": True})
    _add_examples(config, created["uuid"])
    trained = classifier.train_classifier(config, created["uuid"], artifact_version="test-v1")
    ...
    artifact = json.loads(_stored_bytes(config, exported["artifact_path"]))
...
def test_export_writes_build_artifact_copy(tmp_path, spam_config):
    config = _config(spam_config)
    created = store.create_classifier(config, {"name": "test", "enabled": True})
    _add_examples(config, created["uuid"])
    classifier.train_classifier(config, created["uuid"], artifact_version="test-v1")
    ...
    managed_bytes = _stored_bytes(config, exported["artifact_path"])
...
def test_export_rejects_artifact_without_training_metrics(tmp_path, spam_config):
    config = _config(spam_config)
    created = store.create_classifier(config, {"name": "test", "enabled": True})
    artifact = {
        "version": "legacy-v1",
...
def test_custom_token_pattern_is_rejected(tmp_path, spam_config):
    config = _config(spam_config)
...
def test_duplicate_artifact_version_is_rejected_before_overwrite(tmp_path, spam_config):
    config = _config(spam_config)
